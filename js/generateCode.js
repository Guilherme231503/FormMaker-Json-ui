async function generateCode() {
  ...
  const generatedJsonString = generateCodeForElements(zip, namespace)
  const jsonData = JSON.parse(generatedJsonString)
  ...
  zip.file(
    "ui/forms/" + namespace + "/" + namespace + ".json",
  ...
  for (let element of elements) {
    if (element.dataset.name == "Form Body") {
      ...
      if (backgroundImage != "") {
        ...
        zip.file(
          "textures/ui/forms/" + namespace + "/form_background.png",
          byteArray,
          { binary: true }
        )
        ...
        zip.file(
          "textures/ui/forms/" + namespace + "/form_background.json",
          JSON.stringify(nineslice, null, 4)
        )
      }
      ...
      headControls.push({
        content: {
          ...
          texture:
            "textures/ui/" +
            (backgroundImage != ""
              ? "forms/" + namespace + "/form_background"
      ...
      if (rawCode !== null) {
        if (element.dataset.name == "Scroll Panel") {
          ...
          rawCode[getJsonElementName(element)].controls[0][
            "scroll_panel@common.scrolling_panel"
          ]["$scrolling_content"] = "" + namespace + "." + childName
